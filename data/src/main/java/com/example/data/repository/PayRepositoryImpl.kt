...
class PayRepositoryImpl @Inject constructor(
    ...
    override suspend fun getPurchaseInfo(): Result<PayInfoModel?> {
        return runCatching {
            payDataSource.getPurchaseInfoData().data?.toResponsePurchaseInfoModel()
        }
    }

    override suspend fun getUserSubsInfo(): Result<PayUserSubsInfoModel?> {
        return runCatching {
            payDataSource.getUserSubsInfoData().data?.toResponseUserSubsInfoModel()
        }
    }
}
